...
if (isset($_POST['response_id']) && isset($_POST['status'])) {
    ...
    if ($status === 'Accepted') {
        ...
        if (!$update_rfq['success']) {
            die(toast("error", "Failed to update RFQ status"));
        }
        ...
        if (!$reject_others['success']) {
            die(toast("error", "Failed to reject other responses"));
        }
        ...
        foreach ($other_vendors as $vendor) {
            $insert_notification = $DB->INSERT("notifications", [
            ...
            if (!$insert_notification['success']) {
                die(toast("error", "Failed to send notification to vendor #{$vendor['vendor_id']}"));
            }
        }
    }
    ...
    $insert_vendor_notification = $DB->INSERT("notifications", [
    ...
    if (!$insert_vendor_notification['success']) {
        die(toast("error", "Failed to notify vendor #{$response['vendor_id']}"));
    }

    // Notify the action user
    ...
    $insert_user_notification = $DB->INSERT("notifications", [
    ...
    if (!$insert_user_notification['success']) {
        die(toast("error", "Failed to notify action user #" . AUTH_USER_ID));
    }
